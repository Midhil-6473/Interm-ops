# Interm-Ops -- AI Internship Search Pipeline
...
- `{"status": "update-available", "local": "1.0.0", "remote": "1.1.0", "changelog": "..."}` → tell the user:
  > "interm-ops update available (v{local} → v{remote}). Your data (resume, profile, tracker, reports) will NOT be touched. Want me to update?"
...
The user can also say "check for updates" or "update interm-ops" at any time to force a check.
To rollback: `node update-system.mjs rollback`

## What is interm-ops

AI-powered, CLI-agnostic internship search automation: pipeline tracking, offer evaluation, resume generation, portal scanning, batch processing. Runs on any AI coding CLI that follows the [open agent skill standard](https://agentskills.io) (Claude Code, Codex, Gemini, OpenCode, Qwen, Copilot, Kimi, Antigravity CLI).
...
| `templates/resume-template.tex` | LaTeX/Overleaf template for resumes |
| `generate-pdf.mjs` | Playwright: HTML to PDF |
| `generate-latex.mjs` | LaTeX resume validator + pdflatex compiler |
...
Output: `{"onboardingNeeded": <bool>, "missing": [...], "warnings": [...]}`, where `missing` lists whichever of `resume.md`, `config/profile.yml`, `modes/_profile.md`, `portals.yml` are absent. `warnings` is reserved for non-blocking setup signals.
...
- **If, after that, `onboardingNeeded` is still true (any of `resume.md` / `config/profile.yml` / `portals.yml` is missing), enter onboarding mode.** Do NOT proceed with evaluations, scans, or any other mode until the basics are in place. Guide the user step by step:
...
> - Run `/interm-ops scan` (or `/interm-ops-scan` if using OpenCode) to search portals
> - Run `/interm-ops` to see all commands
>
...
If the user accepts, use the `/loop` or `/schedule` skill (if available) to set up a recurring `/interm-ops scan` (or `/interm-ops-scan` if using OpenCode). If those aren't available, suggest adding a cron job or remind them to run `/interm-ops scan` periodically.
...
- Node.js (mjs modules), Playwright (PDF + scraping), YAML (config), HTML/CSS (template), Markdown (data), Canva MCP (optional visual resume)
