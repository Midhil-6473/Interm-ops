...
`Interm-ops` is a local job-search automation workspace built on top of the `career-ops` engine and customized for internship and early-career opportunity management.
...
1. Add personal context such as CV, profile, preferences, and proof points.
...
Create `cv.md` in the project root and write your CV in markdown.
...
- through an AI coding CLI using the `career-ops` command namespace
...
4. Generate tailored CVs only for good-fit roles.
...
- `cv-sync-check.mjs` - checks consistency across CV-related files
...
- `build-cv-latex.mjs` - LaTeX build support
...
- do not invent experience in generated CVs
