#!/usr/bin/env node
// interm-ops scaffolder — one-command install.
// Clones the repo at the latest release tag and installs dependencies.
// It deliberately does NOT create resume.md / config/profile.yml / portals.yml:
...
const REPO = "https://github.com/santifer/interm-ops.git";
const LATEST_RELEASE = "https://api.github.com/repos/santifer/interm-ops/releases/latest";
const NPM = process.platform === "win32" ? "npm.cmd" : "npm";

// interm-ops is AI-agnostic: every one of these CLIs reads AGENTS.md and works
...
const USAGE = `interm-ops — set up an AI job search workspace.

Usage:
  npx interm-ops init [folder]    Create a new workspace (default: ./interm-ops)

After setup, open your AI coding tool inside the folder and paste a job offer.
Docs: https://github.com/santifer/interm-ops`;
...
async function latestTag() {
  try {
    const res = await fetch(LATEST_RELEASE, {
      headers: { "User-Agent": "interm-ops-cli", Accept: "application/vnd.github+json" },
    });
...
async function main() {
  ...
  const target = dirArg || "interm-ops";
  ...
  // Pretty path for messages: "./interm-ops" for relative, as-is for absolute.
  ...
  console.log(`\n→ Cloning interm-ops${tag ? ` @ ${tag}` : ""} into ${display} ...`);
  ...
  // 3. Next steps. We do NOT scaffold resume.md / profile.yml / portals.yml here:
  ...
  console.log(`\n✓ interm-ops is ready in ${display}\n`);
  ...
  console.log("\ninterm-ops is AI-agnostic — Claude Code, Gemini, Codex, Qwen, OpenCode and Copilot all work.");
