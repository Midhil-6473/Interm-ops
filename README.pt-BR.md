...
&nbsp;
<a href="https://www.npmjs.com/package/@santifer/interm-ops"><img src="https://img.shields.io/npm/dt/@santifer/interm-ops?style=for-the-badge&logo=npm&color=CB3837&label=npx%20installs" alt="npm installs"></a></p>
...
> **Importante: isso NÃO é uma ferramenta de disparo em massa.** Interm-ops é um filtro -- ajuda você a encontrar as poucas vagas que realmente valem seu tempo entre centenas. O sistema recomenda fortemente não se candidatar a nada com nota abaixo de 4.0/5. Seu tempo é valioso, e o do recrutador também. Sempre revise antes de enviar.

Interm-ops é agentic: Claude Code navega páginas de carreira com Playwright, avalia aderência comparando seu CV com a descrição da vaga (não por simples correspondência de palavras-chave) e adapta seu currículo para cada vaga.
...
Construído por alguém que usou isso para avaliar 740+ vagas, gerar 100+ CVs personalizados e conquistar uma posição de Head of Applied AI. [Leia o estudo de caso completo](https://santifer.io/interm-ops-system).
...
Isso clona o último release em `./interm-ops` e instala as dependências. Depois:
...
**No primeiro uso, o interm-ops conduz você pela configuração — seu CV, perfil e vagas-alvo — apenas conversando. Nada para editar à mão.**
...
Interm-ops é um único comando slash com múltiplos modos:

```
/interm-ops                → Mostrar todos os comandos disponíveis
/interm-ops {cole um JD}   → Auto-pipeline completo (avaliar + PDF + tracker)
/interm-ops scan           → Escanear portais por novas vagas
/interm-ops pdf            → Gerar CV otimizado para ATS
/interm-ops batch          → Avaliar múltiplas vagas em lote
/interm-ops tracker        → Ver status das candidaturas
/interm-ops apply          → Preencher formulários de candidatura com IA
/interm-ops pipeline       → Processar URLs pendentes
/interm-ops contacto       → Mensagem de outreach no LinkedIn
/interm-ops deep           → Pesquisa aprofundada da empresa
/interm-ops training       → Avaliar um curso/certificação
/interm-ops project        → Avaliar um projeto de portfólio
```

Ou apenas cole uma URL ou descrição de vaga diretamente -- interm-ops detecta automaticamente e roda o pipeline completo.
...
├── resume.md                        # Seu CV (crie este arquivo)
...
│   └── ...
├── templates/
│   ├── resume-template.html         # Template de CV otimizado para ATS
...
Sou o Santiago -- Head of Applied AI, ex-fundador (criei e vendi uma empresa que ainda opera com meu nome). Eu construí o interm-ops para gerenciar minha própria busca de emprego. Funcionou: usei o sistema para conquistar meu cargo atual.
...
**interm-ops é uma ferramenta local e open source — NÃO é um serviço hospedado.** Ao usar este software, você reconhece que:
