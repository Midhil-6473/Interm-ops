...
Export a tailored, ATS-optimized resume as a `.tex` file and compile it to PDF via `tectonic` or `pdflatex`.

## Pipeline

1. Read `resume.md` as source of truth
...
5. Detect JD language → resume language (EN default)
...
11. Build a JSON payload (see schema below) and write to `/tmp/resume-{candidate}-{company}.json`
12. Run: `node build-resume-latex.mjs /tmp/resume-{candidate}-{company}.json output/resume-{candidate}-{company}-{YYYY-MM-DD}.tex`
13. Run: `node generate-latex.mjs output/resume-{candidate}-{company}-{YYYY-MM-DD}.tex output/resume-{candidate}-{company}-{YYYY-MM-DD}.pdf`
...
Write a JSON file with this structure. `build-resume-latex.mjs` handles template merge and LaTeX escaping — no need to escape special characters yourself.
...
| `education[].institution` | string | From resume.md Education |
...
| `experience[].company` | string | From resume.md Experience |
...
| `projects[].name` | string | From resume.md Projects |
...
`build-resume-latex.mjs` automatically escapes all user-supplied text before insertion:
