# interm-ops -- Modos em Português BR (`modes/pt/`)

Esta pasta contém as traduções em português brasileiro dos principais modos do interm-ops para candidatos que buscam vagas no mercado brasileiro ou em empresas que operam em português.
...
O interm-ops não tem um "switch de idioma" como flag de código. Em vez disso, existem dois caminhos:
...
Os modos usam português tech brasileiro, como se fala em times de engenharia reais em São Paulo, Florianópolis ou Belo Horizonte: texto corrido em português, termos técnicos em inglês onde são de uso comum. Nada de traduzir "pipeline" para "tubulação" ou "resume.md" para "curriculo.md".
