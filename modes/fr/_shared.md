# Contexte partage -- interm-ops (Francais)

<!-- ============================================================
     PERSONNALISATION DE CE FICHIER
     ============================================================
     Ce fichier contient le contexte partage pour tous les modes
     interm-ops en version francaise. Avant d'utiliser interm-ops, tu DOIS :
     1. Remplir config/profile.yml avec tes informations personnelles
     2. Creer resume.md a la racine du projet (CV en Markdown)
...
|---------|--------|-------|
| resume.md | `resume.md` (racine du projet) | TOUJOURS |
...
**REGLE : Ne JAMAIS coder en dur des metriques issues des proof points.** Les lire depuis `resume.md` et `article-digest.md` au moment de l'evaluation.
**REGLE : Pour les metriques d'articles/projets, `article-digest.md` a priorite sur `resume.md`** (`resume.md` peut contenir des chiffres plus anciens).
...
> **Metriques concretes : les lire depuis `resume.md` et `article-digest.md` au moment de l'evaluation. JAMAIS les coder en dur ici.**

| Si le role est... | Mettre en avant chez le candidat... | Sources de proof points |
|-------------------|-------------------------------------|-------------------------|
| Platform / LLMOps | Experience production, observability, evals, closed-loop | article-digest.md + resume.md |
| Agentic / Automation | Orchestration multi-agent, HITL, fiabilite, couts | article-digest.md + resume.md |
| Technical AI PM | Product discovery, PRDs, metriques, gestion des parties prenantes | resume.md + article-digest.md |
| Solutions Architect | Conception systeme, integrations, pret pour l'entreprise | article-digest.md + resume.md |
| Forward Deployed Engineer | Livraison rapide, proximite client, prototype a production | resume.md + article-digest.md |
| AI Transformation Lead | Conduite du changement, enablement d'equipe, adoption | resume.md + article-digest.md |
...
2. Modifier `resume.md` ou les fichiers portfolio
...
1. Lire `resume.md` et `article-digest.md` (si existant) avant d'evaluer une offre
1b. **Premiere evaluation de chaque session :** Lancer `node resume-sync-check.mjs` via Bash. En cas d'alertes, prevenir le candidat
