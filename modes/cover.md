...
Works in two modes:
- **Slug mode:** `/interm-ops cover {slug}` — loads the existing evaluation report draft as a starting point
- **Paste mode:** `/interm-ops cover` or JD pasted directly — starts from scratch
...
- `candidate.credentials` (derive from resume.md Education + Certifications if not in profile.yml)
- `cover_letter.notice_period_days` (default: omit if key absent)
- `cover_letter.primary_domain` (default: infer from resume.md if absent)
...
Read `article-digest.md` if it exists — supplementary proof points and metrics take precedence over resume.md where they overlap.
...
- Content stays from resume.md — only vocabulary shifts
...
## Step 7 — Achievement selection (from resume.md only)

Select 4-5 achievement bullets from `resume.md` only (`article-digest.md` may be read for context but is not a source of achievement bullets):
1. Read all bullet points across all roles in resume.md
...
4. Use the exact wording and metrics from resume.md — never paraphrase or invent
...
Years of experience, current/most recent role, domain. Read from resume.md summary.
...
When invoked as `/interm-ops cover {slug}`:
