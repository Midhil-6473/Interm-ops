...
1. Read `resume.md` as the source of truth
...
14. Write HTML to `/tmp/resume-{candidate}-{company}.html`
15. Execute: `node generate-pdf.mjs /tmp/resume-{candidate}-{company}.html output/resume-{candidate}-{company}-{YYYY-MM-DD}.pdf --format={letter|a4}`
...
Use the template in `resume-template.html`. Replace the `{{...}}` placeholders with personalized content:

| Placeholder | Content |
|-------------|-----------|
| `{{LANG}}` | resume language code (e.g. `en`, `es`, `ja`, `ar`). Drives language-specific CSS in the template: `ja` enables a CJK font fallback so Japanese renders instead of tofu (□); `ar` enables RTL + Arabic fonts. Use the BCP-47/ISO-639 code that matches the resume language. |
...
## Canva resume Generation (optional)

If `config/profile.yml` has `resume.canva_resume_design_id` set, offer the user a choice before generating:
- **"HTML/PDF (fast, ATS-optimized)"** — existing flow above
- **"Canva resume (visual, design-preserving)"** — new flow below

If the user has no `resume.canva_resume_design_id`, skip this prompt and use the HTML/PDF flow.

### Canva workflow

#### Step 1 — Duplicate the base design

a. `export-design` the base design (using `resume.canva_resume_design_id`) as PDF → get download URL
...
b. Map text elements to resume sections by content matching:
   - Look for the candidate's name → header section
   - Look for "Summary" or "Professional Summary" → summary section
   - Look for company names from resume.md → experience sections
...
b. **IMMEDIATELY** download the PDF using Bash:
   ```bash
   curl -sL -o "output/resume-{candidate}-{company}-canva-{YYYY-MM-DD}.pdf" "{download_url}"
   ```
   The export URL is a pre-signed S3 link that expires in ~2 hours. Download it right away.
c. Verify the download:
   ```bash
   file output/resume-{candidate}-{company}-canva-{YYYY-MM-DD}.pdf
   ```
...
After generating the resume PDF, offer to generate a cover letter:

```text
Resume PDF generated: output/{path}

Want a cover letter for this role too?
- Say "yes" or "cover letter" to generate one now
- Or run `/interm-ops cover {slug}` later
```
