# Общий контекст — interm-ops (Русский)
...
<!-- ============================================================
     ...
     Перед использованием interm-ops убедитесь:
     1. config/profile.yml заполнен вашими данными
     2. resume.md создан в корне проекта (резюме в Markdown)
...
| Файл | Путь | Когда |
|------|------|-------|
| resume.md | `resume.md` (корень проекта) | ВСЕГДА |
...
**ПРАВИЛО: НИКОГДА не хардкодить метрики из proof points.** Читать их из resume.md + article-digest.md во время оценки.
**ПРАВИЛО: Для метрик статей/проектов `article-digest.md` имеет приоритет над `resume.md`.**
...
> **Конкретные метрики: читать во время оценки из `resume.md` и `article-digest.md`. НИКОГДА не хардкодить.**

| Если роль... | Подчеркнуть о кандидате... | Источники proof points |
|--------------|---------------------------|------------------------|
| Platform / LLMOps | Продакшн-системы, observability, evals | article-digest.md + resume.md |
| Agentic / Automation | Мульти-агентная оркестрация, HITL, надёжность | article-digest.md + resume.md |
| Technical AI PM | Product discovery, PRD, метрики | resume.md + article-digest.md |
| Solutions Architect | Системный дизайн, интеграции, enterprise-ready | article-digest.md + resume.md |
| Forward Deployed Engineer | Быстрая поставка, клиентский фокус, прототип → прод | resume.md + article-digest.md |
| Backend-разработчик | Highload, микросервисы, масштабирование | resume.md |
| DevOps/SRE | Инфраструктура, автоматизация, мониторинг | resume.md |
| Тимлид / EM | Лидерство, найм, техническая стратегия | resume.md |
...
2. Модифицировать resume.md или файлы портфолио
...
1. Читать resume.md, _profile.md и article-digest.md (если есть) перед оценкой
1b. **Первая оценка в сессии:** Запустить `node resume-sync-check.mjs`. Если предупреждения — уведомить пользователя.
