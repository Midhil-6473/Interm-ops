...
- `resume.md`   CV for proof points in drafts
...
> "No active applications to follow up on. Apply to some roles first with `/interm-ops` and come back when they're aging."
...
2. Read `resume.md` for proof points
...
2. **Sentence 2:** One concrete value-add from the report's Block B match or a proof point from resume.md. Quantify if possible.
...
- Suggest the user run `/interm-ops contacto {company}` to find the right person first
...
> - Trying a different contact via `/interm-ops contacto`
...
**To:** {email or "No contact found   run `/interm-ops contacto` first"}
