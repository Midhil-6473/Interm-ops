...
0. **Cover letter:** If the form allows it, ALWAYS include one. Same visual design as the resume. JD quotes mapped to proof points. 1 page max.
...
3. Cite exact lines from the resume when matching
...
| Edit | Update tracker |
| Canva MCP | Optional visual resume generation. Duplicate base design, edit text, export PDF. Requires `resume.canva_resume_design_id` in profile.yml. |
...
- **Style calibration applies to tone and structure only.** Do not import content, claims, or metrics from samples into resumes, reports, or evaluations.
