# interm-ops -- 日本語モード (`modes/ja/`)

このフォルダには、日本市場または日本語で運営される企業での求職活動を行う候補者向けに、interm-ops の主要モードを日本語に翻訳したファイルが含まれています。
...
interm-ops にはコード上の「言語スイッチ」フラグはありません。代わりに 2 つの方法があります：
...
モードは、東京・大阪・福岡の実際のエンジニアリングチームで使われているような自然な日本語テック表現を使います：地の文は日本語、定着しているテック用語は英語のままです。「pipeline」を「配管」と訳したり、「resume.md」を「履歴書.md」にしたりはしません。
