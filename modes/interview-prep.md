...
4. **CV** at `resume.md` + `article-digest.md` — read for proof points
...
- **"Walk me through your resume / why are you looking?"** — 60–90s narrative anchored to `modes/_profile.md` narrative + the role's archetype.
...
- **Background red flags** — gaps, transitions, unusual elements from `resume.md` + `_profile.md`. Honest, specific, forward-looking framing — never defensive.
...
- **Technical questions** (system design, coding, architecture, domain) — for each: the question, source, and what a strong answer looks like for this candidate specifically (reference resume proof points).
...
For each gap, suggest: "You need a story about {topic}. Consider: {specific experience from resume.md that could become a STAR+R story}."
...
- **What to lead with**: stack-relevant proof points from `resume.md` / `article-digest.md`.
