 ...
 *
 * Usage:
 *   node interm-ops/generate-pdf.mjs <input.html> <output.pdf> [--format=letter|a4]
 *
  ...
  for (let i = 1; i < renderedComparable.length; i++) {
    ...
    if (sourcePositions.get(current.key) < sourcePositions.get(previous.key)) {
      ...
      const sourceOrder = source
        ...
        .join(' -> ');
      throw new Error(`resume section order diverges from resume.md: rendered ${renderedOrder}; resume.md ${sourceOrder}`);
    }
  }
}
...
async function generatePDF() {
  ...
  let cvMarkdown = '';
  try {
    cvMarkdown = await readFile(resolve(__dirname, 'resume.md'), 'utf-8');
  } catch (err) {
