...
const TEMPLATE_PATH = resolve(__dirname, 'templates', 'resume-template.tex');
...
async function main() {
  ...
  if (args.length === 0 || args.includes('--help')) {
    console.error('Usage:');
    console.error('  node build-resume-latex.mjs <input.json> <output.tex>');
    console.error('  node build-resume-latex.mjs --test');
    process.exit(1);
  }
  ...
  if (!inputPath || !outputPath) {
    console.error('Usage: node build-resume-latex.mjs <input.json> <output.tex>');
    process.exit(1);
  }
...
async function runSelfTest() {
  ...
  const testOutput = '/tmp/build-resume-latex-test.tex';
  const raw = JSON.stringify(sample, null, 2);
  const tmpInput = '/tmp/build-resume-latex-test-input.json';
