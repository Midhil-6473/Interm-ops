...
<p align="center">
  <a href="https://trendshift.io/repositories/25195" target="_blank"><img src="https://trendshift.io/api/badge/repositories/25195" alt="santifer%2Finterm-ops | Trendshift" style="width: 250px; height: 55px;" width="250" height="55"/></a>
</p>
...
Career-Ops ([interm-ops.org](https://interm-ops.org), znany też jako **careerops**) zamienia dowolne AI CLI w pełne centrum dowodzenia poszukiwaniem pracy. Zamiast ręcznego śledzenia aplikacji w arkuszu kalkulacyjnym, dostajesz pipeline zasilany AI, który:
...
> **Ważne: to NIE jest narzędzie do masowego wysyłania aplikacji.** Interm-ops to filtr — pomaga znaleźć te kilka ofert wartych twojego czasu spośród setek. System stanowczo odradza aplikowanie na oferty z oceną poniżej 4.0/5. Twój czas jest cenny, podobnie jak czas rekrutera. Zawsze sprawdzaj przed wysłaniem.

Interm-ops działa agentowo: Claude Code nawiguje po stronach kariery z Playwright, ocenia dopasowanie rozumując nad twoim CV kontra opis stanowiska (nie przez dopasowanie słów kluczowych) i dostosowuje CV do każdego ogłoszenia.
...
Zbudowany przez kogoś, kto użył go do oceny 740+ ofert pracy, wygenerowania 100+ spersonalizowanych CV i zdobycia roli Head of Applied AI. [Przeczytaj pełne case study](https://santifer.io/interm-ops-system).
...
To sklonuje najnowszą wersję do `./interm-ops` i zainstaluje zależności. Następnie:
...
**Przy pierwszym uruchomieniu interm-ops przeprowadza Cię przez konfigurację — CV, profil i docelowe stanowiska — wyłącznie przez rozmowę. Nic nie trzeba edytować ręcznie.**
...
Interm-ops to jedna komenda slash z wieloma trybami:

```text
/interm-ops                    → Pokaż wszystkie dostępne komendy
/interm-ops {wklej ofertę}     → Pełny auto-pipeline (ocena + PDF + tracker)
/interm-ops scan               → Skanuj portale w poszukiwaniu nowych ofert
/interm-ops pdf                → Generuj CV zoptymalizowane pod ATS
/interm-ops batch              → Wsadowa ocena wielu ofert
/interm-ops tracker            → Podgląd statusu aplikacji
/interm-ops apply              → Wypełnianie formularzy aplikacyjnych z AI
/interm-ops pipeline           → Przetwarzanie kolejki URL
/interm-ops contacto           → Wiadomość na LinkedIn
/interm-ops deep               → Szczegółowe badanie firmy
/interm-ops training           → Ocena kursu/certyfikatu
/interm-ops project            → Ocena projektu portfolio
```

Możesz też po prostu wkleić URL oferty lub jej treść — interm-ops automatycznie to wykryje i uruchomi pełny pipeline.
...
Interm-ops obsługuje główne polskie portale IT. Dwa z nich — JustJoin.it i NoFluffJobs — mają publiczne API i mogą być zintegrowane jako źródła Level 0 (zero tokenów, brak WebSearch, świeże dane w czasie skanowania). Pozostałe portale wymagają weryfikacji ręcznej lub przez Playwright.
...
Interm-ops uwzględnia specyfikę polskiego rynku pracy przy ocenianiu ofert:
...
├── resume.md                        # Twoje CV (utwórz ten plik)
...
│   └── ...
├── templates/
│   ├── resume-template.html         # Szablon CV zoptymalizowany pod ATS
...
Jestem Santiago — Head of Applied AI, były founder (zbudowałem i sprzedałem firmę, która nadal działa z moim nazwiskiem). Zbudowałem interm-ops do zarządzania własnym poszukiwaniem pracy. Zadziałało: użyłem go do zdobycia swojej obecnej roli.
...
**interm-ops to lokalne narzędzie open source, NIE usługa hostingowa.** Korzystając z tego oprogramowania, potwierdzasz:
...
<a href="https://github.com/santifer/interm-ops/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=santifer/interm-ops" alt="Współtwórcy" />
</a>

Znalazłeś pracę dzięki interm-ops? [Podziel się swoją historią!](https://github.com/santifer/interm-ops/issues/new?template=i-got-hired.yml)

## Licencja i znak towarowy

Kod jest licencjonowany na [MIT](LICENSE). Nazwa i marka „interm-ops" są regulowane przez [Politykę Znaków Towarowych](TRADEMARK.md) — dozwolone dla użytku społecznościowego, zastrzeżone dla komercyjnego nazewnictwa produktów i endorsementu.
