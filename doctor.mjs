...
/**
 * doctor.mjs Ã¢â‚¬â€ Setup validation for interm-ops
  ...
  try {
    ...
    return {
      ...
      fix: [
        ...
        'Required for PDF generation and --verify liveness checks, but not for the zero-token internship scanner',
      ],
    };
  } catch {
    return {
      warn: true,
      label: 'Playwright package/browser not ready',
      fix: [
...
function checkPlaywrightMcp(root) {
  ...
  return {
    ...
    fix: [
      ...
      'may return empty or stale content. Tracking: https://github.com/santifer/interm-ops/issues/506',
    ],
  };
}
...
const USER_LAYER_PREREQS = [
  {
    ...
    fix: [
      ...
      'See examples/ for reference resumes',
    ],
  },
  {
...
async function main() {
  ...
  for (const result of checks) {
    ...
    if (result.warn) {
      warnings++;
      console.log(`${yellow('[warn]')} ${result.label}`);
