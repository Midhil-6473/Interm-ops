...
<p align="center">
  <a href="https://trendshift.io/repositories/25195" target="_blank"><img src="https://trendshift.io/api/badge/repositories/25195" alt="santifer%2Finterm-ops | Trendshift" style="width: 245px; height: 54px; vertical-align: middle;" width="245" height="54"/></a>
</p>
...
<p align="center">
  <a href="https://github.com/santifer/interm-ops/releases/latest"><img src="https://img.shields.io/badge/release-v1.9.0-2ea44f?style=for-the-badge&labelColor=2b3137" alt="Dernière version"></a>
</p>
...
> **Important : Il ne s'agit PAS d'un outil pour postuler en masse de manière abusive (spray-and-pray).** Interm-ops est un filtre — il vous aide à identifier les rares offres qui méritent votre temps parmi des centaines d'autres. Le système recommande fortement de ne pas postuler aux offres ayant un score inférieur à 4,0/5. Votre temps est précieux, tout comme celui des recruteurs. Relisez toujours avant d'envoyer.

Interm-ops est agentic : Claude Code navigue sur les pages carrières avec Playwright, évalue l'adéquation en analysant votre CV par rapport à la description du poste (et non par simple correspondance de mots-clés), et adapte votre CV pour chaque offre.
...
Conçu par quelqu'un qui l'a utilisé pour évaluer plus de 740 offres d'emploi, générer plus de 100 CV personnalisés et décrocher un poste de Head of Applied AI. [Lire l'étude de cas complète (en anglais)](https://santifer.io/interm-ops-system).
...
Cette commande clone la dernière version dans `./interm-ops` et installe les dépendances. Ensuite :
...
**Lors du premier lancement, interm-ops vous guide à travers la configuration — votre CV, votre profil et vos rôles cibles — simplement par chat. Rien à modifier à la main.**
...
Interm-ops prend en charge [Gemini CLI](https://github.com/google-gemini/gemini-cli) nativement, de la même manière qu'il supporte Claude Code et OpenCode. Les 15 commandes slash sont disponibles, utilisant la même logique d'évaluation `modes/*.md`.
...
# 3. Exécutez dans le dossier interm-ops
cd interm-ops
gemini

# 4. Utilisez la commande unifiée /interm-ops avec ses sous-commandes :
...
Le fichier `GEMINI.md` est chargé automatiquement comme contexte. Toutes les sous-commandes sont routées via la définition unifiée `.agents/skills/interm-ops/SKILL.md`.
...
Interm-ops est accessible via une commande slash unique avec plusieurs modes :

```
/interm-ops                → Afficher toutes les commandes disponibles
/interm-ops {coller JD}    → Pipeline automatique complet (évaluation + PDF + tracker)
/interm-ops scan           → Scanner les portails pour de nouvelles offres
/interm-ops pdf            → Générer un CV optimisé pour les ATS
/interm-ops batch          → Évaluer plusieurs offres en lot
/interm-ops tracker        → Consulter l'état des candidatures
/interm-ops apply          → Remplir des formulaires de candidature avec l'IA
/interm-ops pipeline       → Traiter les URL en attente
/interm-ops contacto       → Message d'approche LinkedIn
/interm-ops deep           → Recherche approfondie sur une entreprise
/interm-ops training       → Évaluer une formation/certification
/interm-ops project        → Évaluer un projet de portfolio
```

Ou collez simplement l'URL ou la description d'un emploi — interm-ops le détecte automatiquement et lance le pipeline complet.
...
│   └── ...
├── templates/
│   ├── resume-template.html         # Modèle de CV optimisé pour les ATS
...
Je m'appelle Santiago — Head of Applied AI, ancien fondateur (j'ai créé et vendu une entreprise qui fonctionne toujours sous mon nom). J'ai conçu interm-ops pour gérer ma propre recherche d'emploi. Cela a fonctionné : je l'ai utilisé pour décrocher mon poste actuel.

Mon portfolio et mes autres projets open source → [santifer.io](https://santifer.io)

## Historique des étoiles

<a href="https://www.star-history.com/?repos=santifer%2Finterm-ops&type=timeline&legend=top-left">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/chart?repos=santifer/interm-ops&type=timeline&theme=dark&legend=top-left" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/chart?repos=santifer/interm-ops&type=timeline&legend=top-left" />
   <img alt="Star History Chart" src="https://api.star-history.com/chart?repos=santifer/interm-ops&type=timeline&legend=top-left" />
 </picture>
</a>

## Clause de non-responsabilité

**interm-ops est un outil local et open source, pas un service hébergé.** En utilisant ce logiciel, vous reconnaissez que :
...
<a href="https://github.com/santifer/interm-ops/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=santifer/interm-ops" />
</a>

Vous avez été embauché grâce à interm-ops ? [Partagez votre histoire ! (en anglais)](https://github.com/santifer/interm-ops/issues/new?template=i-got-hired.yml)

## Licence et marques déposées

Le code est distribué sous [Licence MIT](LICENSE). Le nom et la marque "interm-ops" sont régis par la [Politique de marques déposées](TRADEMARK.md), qui autorise l'usage par la communauté mais le réserve pour les produits commerciaux et les promotions.
