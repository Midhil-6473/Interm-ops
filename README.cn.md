...
> **重要：这不是海投工具。** interm-ops 是一个过滤器，帮你从数百个职位里找出真正值得投入时间的少数机会。系统强烈建议不要申请评分低于 4.0/5 的职位。你的时间很宝贵，招聘方的时间也一样。提交前一定要自己复核。

interm-ops 具备代理式工作能力：Claude Code 会用 Playwright 浏览招聘页面，通过推理你的简历与职位描述是否匹配来评估契合度，而不是只做关键词匹配；同时它也会根据每个职位调整你的简历。

> **提醒：最开始几次评估不会特别准。** 系统还不了解你。请给它更多上下文，比如你的简历、职业故事、成果证明、个人偏好、擅长的事、想避开的事。你喂给它的信息越多，它就越准确。把它当成在培养一个新招聘顾问：第一周它需要先了解你，之后就会变得非常有价值。

这个系统的作者曾用它评估 740 多个职位、生成 100 多份定制简历，并拿到一份 Head of Applied AI 的工作。[阅读完整案例研究](https://santifer.io/interm-ops-system)。
...
这会把最新版本克隆到 `./interm-ops` 并安装依赖。然后：
...
**首次启动时，interm-ops 会通过对话带你完成设置 —— 你的简历、个人档案和目标职位 —— 完全无需手动编辑任何文件。**
...
interm-ops 原生支持 [Gemini CLI](https://github.com/google-gemini/gemini-cli) —— 与 Claude Code 和 OpenCode 的支持方式相同。所有 15 个斜杠命令均可使用，并基于相同的 `modes/*.md` 评估逻辑。
...
# 2. 在 interm-ops 目录中运行 —— 首次启动时使用你的 Google 账号登录（免费）完成认证
cd interm-ops
gemini

# 3. 使用统一的 /interm-ops 命令及其子命令：
/interm-ops "Anthropic 的资深 AI 工程师..."
/interm-ops pipeline
/interm-ops scan
/interm-ops pdf
/interm-ops tracker
...
`GEMINI.md` 文件会自动作为上下文加载。所有子命令都通过统一的 `.agents/skills/interm-ops/SKILL.md` 定义进行路由。
...
interm-ops 是一个单一斜杠命令，带有多种模式：

```
/interm-ops                → 显示所有可用命令
/interm-ops {粘贴职位描述}  → 完整自动管道（评估 + PDF + 追踪）
/interm-ops scan           → 扫描平台上的新职位
/interm-ops pdf            → 生成 ATS 优化简历
/interm-ops batch          → 批量评估多个职位
/interm-ops tracker        → 查看申请状态
/interm-ops apply          → 用 AI 协助填写申请表
/interm-ops pipeline       → 处理待办 URL
/interm-ops contacto       → 生成 LinkedIn 外联消息
/interm-ops deep           → 深度公司研究
/interm-ops training       → 评估课程/证书
/interm-ops project        → 评估作品集项目
```

或者直接粘贴职位 URL 或职位描述，interm-ops 会自动识别并运行完整流程。
...
│   └── ...
├── templates/
│   ├── resume-template.html         # ATS 优化简历模板
...
我是 Santiago，现任 Head of Applied AI，也曾是一名创业者（创建并出售过一家公司，那家公司至今仍以我的名字运营）。我构建 interm-ops 是为了管理我自己的求职流程，而它确实奏效了：我用它拿到了现在这份工作。
...
**interm-ops 是一个本地开源工具，不是托管服务。** 使用本软件即表示你确认：
...
通过 interm-ops 成功入职？[分享你的故事！](https://github.com/santifer/interm-ops/issues/new?template=i-got-hired.yml)

## 许可证与商标

代码以 [MIT](LICENSE) 许可证授权。"interm-ops" 名称及品牌受 [商标政策](TRADEMARK.md) 约束 —— 允许社区使用，商业产品命名和背书需保留权利。
