...
Reference files that demonstrate interm-ops data formats and conventions. None of these are used at runtime -- they exist so you can see the expected structure before creating your own files.

## Files

| File | Demonstrates |
|------|-------------|
| `resume-example.md` | How to structure `resume.md` -- sections, metrics formatting, and proof-point style for a fictional AI engineer (Alex Chen) |
...
| `dual-track-engineer-instructor/` | Complete profile config for a candidate with two primary archetypes (engineer + instructor), including `resume.md`, `profile.yml`, and a README explaining when and how to use the dual-track pattern |

## Usage

These files are read-only references. To set up your own interm-ops instance:

1. Run `npm run doctor` to check prerequisites.
2. Use `resume-example.md` as a structural guide when writing your `resume.md`.
