...
The archetype table in `_profile.md` determines how offers are scored and resumes are framed. Edit the table to match YOUR career targets:
...
Interm-ops can integrate with external systems via Claude Code hooks. Example hooks:
...
```json
{
  "hooks": {
    "SessionStart": [{
      "hooks": [{
        "type": "command",
        "command": "echo 'Interm-ops session started'"
      }]
    }]
  }
}
