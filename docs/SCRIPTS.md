...
| `npm run build:latex` | `build-resume-latex.mjs` | Build .tex from structured JSON payload |
| `npm run sync-check` | `resume-sync-check.mjs` | Validate resume/profile consistency |
...
Validates that all prerequisites are in place: Node.js >= 18, dependencies installed, Playwright chromium, required files (`resume.md`, `config/profile.yml`, `portals.yml`), fonts directory, and auto-creates `data/`, `output/`, `reports/` if missing.
...
```bash
node build-resume-latex.mjs input.json output.tex
node build-resume-latex.mjs --test
```
...
Validates that the interm-ops setup is internally consistent: `resume.md` exists and is not too short, `config/profile.yml` exists with required fields, no hardcoded metrics in `modes/_shared.md` or `batch/batch-prompt.md`, and `article-digest.md` freshness (warns if older than 30 days).
...
Checks whether a newer version of interm-ops is available upstream. Outputs JSON to stdout:
