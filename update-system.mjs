...
/**
 * update-system.mjs — Safe auto-updater for interm-ops
 *
...
const CANONICAL_REPO = 'https://github.com/santifer/interm-ops.git';
const RAW_VERSION_URL = 'https://raw.githubusercontent.com/santifer/interm-ops/main/VERSION';
const RELEASES_API = 'https://api.github.com/repos/santifer/interm-ops/releases/latest';

// Matches a semver, with or without a leading `v` and an optional
// Release Please component prefix (e.g. `interm-ops-v1.9.0` → `1.9.0`).
...
async function check() {
  ...
  const [rawVersion, releaseRaw] = await Promise.all([
    ...
    curlGet(RELEASES_API, [
      ...
      '--header', 'User-Agent: interm-ops-update-checker',
    ]),
  ]);
