...
/**
 * test-all.mjs — Comprehensive test suite for interm-ops
 *
 ...
 * any freshly cloned interm-ops checkout with only Node.js available.
 *
 ...
 *
 * @param {string} path - Path relative to the interm-ops repository root.
...
const scripts = [
  { name: 'resume-sync-check.mjs', expectExit: 1, allowFail: true }, // fails without resume.md (normal in repo)
  ...
  // their active interm-ops workspace.
...
const systemFiles = [
  ...
  'templates/states.yml', 'templates/resume-template.html',
  '.claude/skills/interm-ops/SKILL.md',
  '.opencode/skills/interm-ops/SKILL.md',
  '.antigravitycli/skills/interm-ops/SKILL.md',
];
...
for (const skillPath of ['.claude/skills/interm-ops/SKILL.md', '.agents/skills/interm-ops/SKILL.md']) {
...
try {
  const tmp = mkdtempSync(join(tmpdir(), 'interm-ops-portals-validator-'));
...
const canonicalSkill = '.agents/skills/interm-ops/SKILL.md';
const symlinks = [
  '.claude/skills/interm-ops/SKILL.md',
  '.opencode/skills/interm-ops/SKILL.md',
  '.antigravitycli/skills/interm-ops/SKILL.md',
...
try {
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), 'interm-ops-migrate-'));
  try {
...
try {
  ...
  const dedupTmp = mkdtempSync(join(tmpdir(), 'interm-ops-dedup-'));
  try {
...
try {
  const mergeTmp = mkdtempSync(join(tmpdir(), 'interm-ops-merge-'));
  try {
...
try {
  const col912Tmp = mkdtempSync(join(tmpdir(), 'interm-ops-merge-912-'));
  try {
...
try {
  const mergeTmp = mkdtempSync(join(tmpdir(), 'interm-ops-merge-lock-'));
  /**
    ...
    const result = new Promise(resolve => {
      const child = spawn(NODE, ['merge-tracker.mjs'], {
        ...
        env: {
          ...
          CAREER_OPS_TRACKER_LOCK: join(mergeTmp, 'interm-ops-merge-tracker-fixture.lock'),
...
try {
  ...
  for (const f of ['resume.md', 'config/profile.yml', 'modes/_profile.md', 'portals.yml']) {
...
if (!sqliteAvailable) {
  ...
  try {
    const idxTmp = mkdtempSync(join(tmpdir(), 'interm-ops-index-'));
    try {
...
try {
  ...
  const hit = sj.detect({ name: 'SJ', careers_url: 'https://solid.jobs/public-api/offers/it?campaign=interm-ops' });
  ...
  const fakeJobs = {
    jobs: [
      { title: 'Senior Dev', url: 'https://solid.jobs/o/abc123/interm-ops', company: 'Acme Corp', locations: ['Warszawa', 'Remote'] },
      { title: 'Junior Dev', url: 'https://solid.jobs/o/def456/interm-ops', company: 'Beta Inc', locations: ['Kraków'] },
    ],
  };
  const parsed = await sj.fetch(
    { name: 'SolidJobs IT', careers_url: 'https://solid.jobs/public-api/offers/it?campaign=interm-ops' },
  ...
  if (parsed[0].title === 'Senior Dev' && parsed[0].url === 'https://solid.jobs/o/abc123/interm-ops') {
  ...
  const noCompanyJobs = { jobs: [{ title: 'Tester', url: 'https://solid.jobs/o/xyz/interm-ops', locations: [] }] };
  ...
  const mixedJobs = {
    jobs: [
      { title: 'Has URL', url: 'https://solid.jobs/o/1/interm-ops', company: 'A', locations: [] },
  ...
  const stringLocJobs = { jobs: [{ title: 'Dev', url: 'https://solid.jobs/o/2/interm-ops', company: 'X', locations: 'Warsaw' }] };
  ...
  const malformedMembers = { jobs: [null, 7, { title: 'OK', url: 'https://solid.jobs/o/3/interm-ops', company: 'Z' }] };
  const safeParsed = await sj.fetch(
    { name: 'SolidJobs IT', careers_url: 'https://solid.jobs/public-api/offers/it?campaign=interm-ops' },
    { transport: 'http', fetchJson: async () => malformedMembers, fetchText: async () => '' },
  );
  if (safeParsed.length === 1 && safeParsed[0].url === 'https://solid.jobs/o/3/interm-ops') {
...
try {
  ...
  // Release Please tags carry the component prefix (interm-ops-v1.9.0); the
  // prefix must be stripped or the releases-API fallback is dead code (#923).
  if (parse('interm-ops-v1.9.0') === '1.9.0') {
    pass('SEMVER_RE parses Release Please component-prefixed tag (interm-ops-v1.9.0 → 1.9.0)');
  } else {
    fail(`SEMVER_RE failed on interm-ops-v1.9.0 (got ${parse('interm-ops-v1.9.0')}) — releases-API fallback is dead code (#923)`);
  }
  ...
  if (parse('interm-ops') === null && parse('v1.9') === null) {
    pass('SEMVER_RE rejects non-semver input');
  } else {
    fail(`SEMVER_RE matched non-semver input (interm-ops → ${parse('interm-ops')}, v1.9 → ${parse('v1.9')})`);
  }
} catch (e) {
...
try {
  ...
  const traversal = await inlineLocalFonts(`<style>src: url('./fonts/../resume.md');</style>`);
  if (traversal.includes(`url('./fonts/../resume.md')`)) {
...
try {
  ...
  const template = readFileSync(join(ROOT, 'templates', 'resume-template.html'), 'utf-8');

  if (/html\[lang="ja"\]\s+body/.test(template)) {
    pass('resume-template.html has a lang="ja" body rule for CJK text');
  } else {
    fail('resume-template.html is missing a lang="ja" font fallback — Japanese CVs render as tofu (□)');
  }
