...
- `{"status": "update-available", "local": "1.0.0", "remote": "1.1.0", "changelog": "..."}` → tell the user:
  > "interm-ops update available (v{local} → v{remote}). Your data (CV, profile, tracker, reports) will NOT be touched. Want me to update?"
...
| `templates/resume-template.html` | HTML template for CVs |
| `templates/resume-template.tex` | LaTeX/Overleaf template for CVs |
...
Instead of registering individual `.toml` files for every slash command, all subcommands are routed through the single unified skill defined in `.agents/skills/interm-ops/SKILL.md`.

You can invoke the command center or any of its modes directly within your CLI:

* `/interm-ops` (Shows the Command Center menu)
* `/interm-ops {JD text or URL}` (Runs the auto-evaluation pipeline)
* `/interm-ops [subcommand]` (Runs a specific subcommand)
...
If the user accepts, use the `/loop` or `/schedule` skill (if available) to set up a recurring `/interm-ops scan` (or `/interm-ops-scan` if using OpenCode). If those aren't available, suggest adding a cron job or remind them to run `/interm-ops scan` (or `/interm-ops-scan` if using OpenCode) periodically.
...
- "Change the CV template design" → edit `templates/resume-template.html`
...
- `resume.md` in project root is the canonical CV
