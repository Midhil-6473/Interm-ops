 ...
 *
 * Run: node interm-ops/merge-tracker.mjs [--dry-run] [--verify]
 */
 ...
 * and use the interm-ops lock-name prefix. Invalid values are ignored and the
...
function resolveTrackerLockDir(envValue, lockKey) {
  ...
  const fallback = join(tmpRoot, `interm-ops-merge-tracker-${lockKey}.lock`);
  ...
  if (!basename(candidate).startsWith('interm-ops-merge-tracker-')) return fallback;
  return candidate;
}
