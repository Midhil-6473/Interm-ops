...
System-layer template files used by interm-ops scripts and modes. These files are auto-updated when you run `npm run update` -- put user customizations in the user-layer files instead (see DATA_CONTRACT.md).

## Files

| File | Used By | Purpose |
|------|---------|---------|
| `resume-template.html` | `generate-pdf.mjs` | HTML/CSS template for ATS-optimized resume PDFs |
| `resume-template.tex` | `generate-latex.mjs` | LaTeX/Overleaf template for ATS-optimized resume PDFs |
...
LaTeX template for Overleaf-compatible resume generation. Based on the [sb2nov/resume](https://github.com/sb2nov/resume) format. Uses placeholder tokens (`{{NAME}}`, `{{EXPERIENCE}}`, `{{PROJECTS}}`, etc.) that the LaTeX pipeline fills at generation time.
...
node generate-latex.mjs output/resume-name-company-date.tex

# Or specify a custom output path
node generate-latex.mjs output/resume-name-company-date.tex output/custom-name.pdf
```
