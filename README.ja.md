...
> **重要: これは「とにかく数を撃つ」ツールではありません。** interm-opsはフィルターです -- 何百もの求人の中から、あなたの時間を割く価値のある数件を見つけ出すためのツールです。本システムは4.0/5未満のスコアの求人への応募を強く非推奨としています。あなたの時間もリクルーターの時間も貴重です。送信前に必ず内容を確認してください。

interm-opsはエージェンティックです: Claude CodeがPlaywrightで求人ページを操作し、（キーワードマッチではなく）あなたのCVと求人票を突き合わせて適合度を推論し、求人ごとにレジュメを最適化します。
...
このシステムは、740件以上の求人を評価し、100件以上のテーラーメイドCVを生成し、Head of Applied AIのポジションを獲得した人物によって作られました。[詳細なケーススタディを読む](https://santifer.io/interm-ops-system)。
...
これにより最新リリースが `./interm-ops` にクローンされ、依存関係がインストールされます。その後:
...
**初回起動時、interm-opsが対話するだけでセットアップ（CV、プロフィール、対象ロール）をご案内します。手で編集するものは何もありません。**
...
interm-opsは複数のモードを持つ単一のスラッシュコマンドです:

```
/interm-ops                → 利用可能なすべてのコマンドを表示
/interm-ops {求人票を貼る}  → 完全自動パイプライン（評価 + PDF + トラッカー）
/interm-ops scan           → ポータルをスキャンして新しい求人を探す
/interm-ops pdf            → ATS最適化CVを生成
/interm-ops batch          → 複数オファーをバッチ評価
/interm-ops tracker        → 応募ステータスを表示
/interm-ops apply          → AIで応募フォームを入力
/interm-ops pipeline       → 保留中のURLを処理
/interm-ops contacto       → LinkedInアウトリーチメッセージ
/interm-ops deep           → 企業の深掘りリサーチ
/interm-ops training       → コース/資格を評価
/interm-ops project        → ポートフォリオプロジェクトを評価
```

または、単に求人URLや記述を直接貼り付けるだけ -- interm-opsが自動検知してフルパイプラインを実行します。
...
│   └── ...
├── templates/
│   ├── resume-template.html         # ATS最適化CVテンプレート
...
Santiagoです -- Head of Applied AI、元創業者（自分の名前を冠した事業を立ち上げて売却、その事業は今も稼働中）。interm-opsは自分自身の求職活動を管理するために作りました。結果、現職を獲得することに成功しました。
...
**interm-opsはローカルで動作するオープンソースツールです — ホステッドサービスではありません。** 本ソフトウェアを使用することにより、以下を承諾したものとみなされます:
