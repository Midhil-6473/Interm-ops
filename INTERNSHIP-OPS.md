...
`internship-ops` is your customized version of `career-ops`.
...
## What Changed For internship-ops

The underlying engine is still `career-ops`, but the user-side configuration is now aimed at internships:
...
Important:
- commands and scripts still use the existing `career-ops` naming so nothing breaks
- this is the safest way to turn the repo into `internship-ops` without rewriting the whole system
...
Paste your current resume into `cv.md`.
...
Once `cv.md` exists, the system can tailor your resume for each internship.
...
This uses your `cv.md` plus the internship context to generate a customized resume.
...
The system can tailor resumes only from what exists in `cv.md` and related proof sources.
...
Right now this repo is functionally customized into `internship-ops` without changing command names.
