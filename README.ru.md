...
> **Важно: это НЕ инструмент для массовой рассылки.** Interm-ops — это фильтр. Он помогает найти те немногие вакансии, которые стоят вашего времени, из сотен. Система настоятельно рекомендует не откликаться на вакансии с оценкой ниже 4.0/5. Ваше время ценно, как и время рекрутера. Всегда проверяйте перед отправкой.
...
Это клонирует последний релиз в `./interm-ops` и устанавливает зависимости. Затем:
...
**При первом запуске interm-ops проведёт тебя через настройку — твоё CV, профиль и целевые роли — просто через диалог. Ничего не нужно править вручную.**
...
Interm-ops — одна slash-команда с множеством режимов:

```
/interm-ops                       → Показать все доступные команды
/interm-ops {вставь описание JD}  → Полный авто-пайплайн (оценка + PDF + трекер)
/interm-ops scan                  → Сканирование порталов
/interm-ops pdf                   → Генерация ATS-резюме
/interm-ops batch                 → Пакетная оценка вакансий
/interm-ops tracker               → Статус откликов
/interm-ops apply                 → Заполнение форм отклика с ИИ
/interm-ops pipeline              → Обработка очереди URL
/interm-ops contacto              → Сообщение в LinkedIn
/interm-ops deep                  → Глубокий анализ компании
/interm-ops training              → Оценка курса/сертификации
/interm-ops project               → Оценка портфолио-проекта
```

Или просто вставь URL вакансии — interm-ops автоматически определит его и запустит полный пайплайн.
...
Interm-ops поддерживает ключевые российские джоб-борды:
...
│   └── ...
├── templates/
│   ├── resume-template.html         # ATS-шаблон резюме
...
**interm-ops — локальный open-source инструмент, НЕ хостинг-сервис.** Используя это ПО, вы подтверждаете:
