...
<p align="center">
  ...
  &nbsp;
  <a href="https://www.npmjs.com/package/@santifer/interm-ops"><img src="https://img.shields.io/npm/dt/@santifer/interm-ops?style=for-the-badge&logo=npm&color=CB3837&label=npx%20installs" alt="npm installs"></a>
</p>
...
> **Важливо: це НЕ інструмент для масової розсилки.** Interm-ops — це фільтр, який допомагає знайти кілька вакансій, вартих вашого часу, серед сотень. Система наполегливо рекомендує не подаватися на вакансії з балом нижче 4.0/5. Ваш час цінний, як і час рекрутера. Завжди перевіряйте все перед подачею заявки.

Interm-ops працює агентно: Claude Code переходить на кар'єрні сторінки за допомогою Playwright, оцінює відповідність, аналізуючи ваше резюме відносно опису вакансії (не за ключовими словами), та адаптує ваше резюме під кожну вакансію.
...
Створено людиною, яка використовувала систему для оцінки 740+ вакансій, генерації 100+ персоналізованих резюме та отримання посади Head of Applied AI. [Читати повний кейс](https://santifer.io/interm-ops-system).
...
Це клонує останній реліз у `./interm-ops` та встановлює залежності. Потім:
...
**Під час першого запуску interm-ops проведе вас через налаштування — ваше резюме, профіль і цільові ролі — просто у форматі діалогу. Нічого не треба редагувати вручну.**
...
Interm-ops — це одна слеш-команда з кількома режимами:

```text
/interm-ops                → Показати всі доступні команди
/interm-ops {вставити JD}  → Повний авто-конвеєр (оцінка + PDF + трекер)
/interm-ops scan           → Сканувати портали на нові вакансії
/interm-ops pdf            → Згенерувати ATS-оптимізоване резюме
/interm-ops batch          → Пакетна оцінка кількох вакансій
/interm-ops tracker        → Переглянути статус заявок
/interm-ops apply          → Заповнити форми заявок з AI
/interm-ops pipeline       → Обробити очікуючі URL
/interm-ops contacto       → Повідомлення для LinkedIn outreach
/interm-ops deep           → Глибоке дослідження компанії
/interm-ops training       → Оцінити курс/сертифікацію
/interm-ops project        → Оцінити портфоліо-проєкт
```

Або просто вставте URL вакансії чи її опис — interm-ops автоматично визначить це та запустить повний конвеєр.
...
├── resume.md                        # Ваше резюме (створіть цей файл)
...
│   └── ...
├── templates/
│   ├── resume-template.html         # ATS-оптимізований шаблон резюме
...
Я Сантьяго — Head of Applied AI, колишній засновник (побудував і продав бізнес, який досі працює під моїм ім'ям). Я створив interm-ops для управління власним пошуком роботи. Це спрацювало: я використав його, щоб отримати свою поточну посаду.

Моє портфоліо та інші open source проєкти → [santifer.io](https://santifer.io)

☕ [Пригостіть мене кавою](https://buymeacoffee.com/santifer), якщо interm-ops допоміг у вашому пошуку роботи.
...
<a href="https://www.star-history.com/?repos=santifer%2Finterm-ops&type=timeline&legend=top-left">
 <picture>
   ...
   <img alt="Графік історії зірок" src="https://api.star-history.com/chart?repos=santifer/interm-ops&type=timeline&legend=top-left" />
 </picture>
</a>

## Застереження

**interm-ops — це локальний інструмент з відкритим кодом, а НЕ хмарний сервіс.** Використовуючи це програмне забезпечення, ви підтверджуєте:
...
<a href="https://github.com/santifer/interm-ops/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=santifer/interm-ops" alt="Контриб'ютори santifer/interm-ops" />
</a>

Отримали роботу завдяки interm-ops? [Поділіться своєю історією!](https://github.com/santifer/interm-ops/issues/new?template=i-got-hired.yml)
